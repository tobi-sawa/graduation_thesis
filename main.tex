...
本方式では，カテゴリ割当て時点，入札時点（タスク分解直後），最終統合後の3段階で精度を確認した．
カテゴリ割当てにより適合率が改善する一方，入札時点は候補APIが多く適合率が低いが，再現率は高めである．
最終統合では整合性を優先するため再現率が低下する傾向が確認された．
各段階の推薦結果を表~\ref{tab:manager-category-results}，表~\ref{tab:manager-bidding-results}，
表~\ref{tab:manager-results}に示す．

\begin{table}[tb]
  \centering
  \caption{マネージャ主導方式（カテゴリ割当て時点）の推薦結果}
  \label{tab:manager-category-results}
  \begin{tabular}{l c}
    \hline
    指標 & 値 \\
    \hline
...
\begin{table}[tb]
  \centering
  \caption{マネージャ主導方式（入札時点）の推薦結果}
  \label{tab:manager-bidding-results}
  \begin{tabular}{l c}
    \hline
    指標 & 値 \\
    \hline
