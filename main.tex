...
加えて，各プロトコルの実行時間と入出力トークン数を計測し，平均値で比較した．
表~\ref{tab:runtime-token-results}に示す．

\begin{table}[tb]
  \centering
  \caption{各方式の実行時間とトークン数（平均）}
  \label{tab:runtime-token-results}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{|l|r|r|r|r|r|}
    \hline
    方式 & 入力トークン & 出力トークン & 総トークン & LLM時間(s) & 総時間(s) \\
    \hline
    マネージャ主導 & 9843.45 & 2716.56 & 12560.01 & 22.43 & 183.87 \\
    \hline
    協調型 & 522411.36 & 71828.16 & 594239.52 & 2757.40 & 1456.35 \\
    \hline
    単一エージェント & 15403.46 & 35469.25 & 50872.71 & 261.04 & 739.67 \\
    \hline
  \end{tabular}%
  }
\end{table}
...
また，実行時間とトークン数については，他の方式と比較して最も効率的であった．総トークン数は約12,560，総実行時間は約183秒であり，マネージャが事前にカテゴリを絞り込むことで，不要な通信や処理を抑制できたためと考えられる．
...
一方，実行時間とトークン数については，3方式の中で最もコストが高くなった．総トークン数は約594,240，総実行時間は約1,456秒に達した．これは，マネージャとコントラクタ間での多段階のやり取りが発生し，各ステップでの入出力が増大したためである．
提案の統合段階における情報損失が影響していると考えられる．

表~\ref{tab:all-recommendation-results}に示す通り，各方式の推薦結果を比較できる．
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|c|c|c|}
    ...
    方式 & 段階 & 適合率 & 再現率 & F1 値 \\
    \cline{1-1}\cline{2-5}
    \multirow{3}{*}{マネージャ主導} & カテゴリ割当て時点 & 0.111 & 0.607 & 0.177 \\
    \cline{2-5}
     & 入札時点 & 0.047 & 0.485 & 0.079 \\
    \cline{2-5}
     & 最終推薦 & 0.159 & 0.298 & 0.193 \\
    \cline{1-1}\cline{2-5}
    \multirow{3}{*}{コントラクタ主導} & カテゴリ割当て時点 & 0.000 & 0.000 & 0.000 \\
    \cline{2-5}
     & 入札時点 & 0.000 & 0.000 & 0.000 \\
    \cline{2-5}
     & 最終推薦 & 0.000 & 0.000 & 0.000 \\
    \cline{1-1}\cline{2-5}
    \multirow{3}{*}{協調型} & カテゴリ割当て時点 & 0.047 & 0.347 & 0.078 \\
    \cline{2-5}
     & 入札時点 & 0.034 & 0.260 & 0.058 \\
    \cline{2-5}
     & 最終推薦 & 0.083 & 0.223 & 0.114 \\
    \cline{1-1}\cline{2-5}
    \multirow{2}{*}{単一エージェント} & カテゴリ割当て時点 & 0.174 & 0.310 & 0.208 \\
    \cline{2-5}
     & 最終推薦 & 0.137 & 0.141 & 0.130 \\
    \cline{1-1}\cline{2-5}
  \end{tabular}
\end{table}
...
一方で，協調型（適合率 0.083，再現率 0.223）は，コントラクタ側でのカテゴリ推定とマネージャ側での統合という柔軟な手続き持つものの，マネージャ主導型ほどの精度には達しなかった．
これは，各コントラクタが局所的な視点でカテゴリ適合性を判断するため，システム全体としての一貫性が保ちにくく，結果としてマネージャが統合時に正解候補を見落とす，あるいは誤った候補を採用する頻度が高まったためと推察される．
コントラクタ主導型に関しては，全コントラクタが要件解釈を行うことによる発散のリスクが顕在化し，有効な推薦集合を形成することが困難であった．
...
推論コスト（トークン数および実行時間）の観点では，マネージャ主導型の優位性が顕著であった．
...
対照的に，協調型やコントラクタ主導型では，マネージャと多数のコントラクタ間でのブロードキャスト的な通信や，多段階のネゴシエーションが発生し，通信量が指数関数的に増大した．特に協調型では，柔軟性を担保するための調整コストが非常に高くつくことが明らかになった．
...
一方，協調型やコントラクタ主導型はコントラクタの自律性を重視し，マネージャの判断ミスを局所的な知識で補完できる可能性を持つが，その代償として統合の複雑さと通信コストが増大する．
...
今後の課題として，本研究のプロトコル別結果を踏まえた改善施策を具体化する必要がある．
まず，入札時点で候補が拡張され適合率が低下する傾向が見られたため，
公告範囲の動的制御や入札の事前フィルタリングにより候補の過剰生成を抑制し，通信量と誤推薦を低減する．
次に，要求の粒度・曖昧さにより有利な方式が異なることが示されたため，
要求タイプ推定に基づくプロトコル自動選択を導入し，要求ごとに最適な分担で推論する．
さらに，タスクの入出力整合性を強化し，
統合段階での不整合や情報損失を検出・修正できる仕組みを整える．
加えて，本研究ではコントラクタエージェントに 1 API のみを割り当てているため，
今後は 1 エージェントに複数 API を持たせた場合の精度・通信量・統合負荷の変化を評価し，
専門性の深さと分担効率の最適なバランスを検証する実験を追加で実施する．
...
\item[コントラクタエージェントの組織化]
単一API単位で専門化したコントラクタ構成を用い，分散保持の有効性と推薦精度への影響を評価した．
複数APIを担当させる設計については今後の課題として位置付けた．
\end{description}

今後の展望として，通信効率化，プロトコル自動選択，タスクの入出力整合性強化などに取り組むことで，
