...
表~\ref{tab:all-recommendation-results}に示す通り，カテゴリ割当て時点では適合率は 0.169，再現率は 0.402，F1 値は 0.214 となった．
入札時点では適合率は 0.015，再現率は 0.510，F1 値は 0.028 となり，適合率が大幅に低下した．
これは，各コントラクタが独立してカテゴリ推定とマッチングを行うため，全体的な一貫性が欠け，多数の無関係な API が候補として提示されたためと考えられる．
最終統合後では適合率は 0.049，再現率は 0.146，F1 値は 0.070 となり，3方式の中で最も低い性能を示した．
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|c|c|c|}
    ...
    \cline{1-1}\cline{2-5}
    \multirow{3}{*}{コントラクタ主導} & カテゴリ割当て時点 & 0.169 & 0.402 & 0.214 \\
    \cline{2-5}
     & 入札時点 & 0.015 & 0.510 & 0.028 \\
    \cline{2-5}
     & 最終推薦 & 0.049 & 0.146 & 0.070 \\
    \cline{1-1}\cline{2-5}
...
コントラクタ主導型（適合率 0.049，再現率 0.146）は，3 方式の中で最も低い精度を示した．
全コントラクタが独立して要件解釈とカテゴリ推定を行うため，判断の発散が極めて大きく，無関連な API が過剰に列挙される結果となった．
各エージェント間の判断基準が統一されず，統合段階で情報損失が大きく発生したためと考えられる．
...
タスク分解の安定性や入札の一貫性にどの程度影響するかを明らかにすることで，
手法の一般性を高めることができる．
